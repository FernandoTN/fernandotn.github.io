...
summary: 'When we built an AI shopping assistant under deadline pressure, our framework choice became a liability. This is the story of switching from LangGraph to LangChain mid-development and what it taught us about framework selection for production agents.'
publishedAt: '2025-12-19'
tags: ['AI Agents', 'LangGraph', 'LangChain', 'Prototyping', 'E-commerce']
featured: false
...
> "During development, we initially implemented the agent using LangGraph but switched to LangChain due to extensive bloat and complexity."
...
The difference in approach is best illustrated through code. Here is a simplified representation of how we handled checkout fallback logic in each framework:

```python
# LangGraph approach (conceptual representation)
...
def create_checkout_graph():
    graph = StateGraph(CheckoutState)
    graph.add_node("api_checkout", api_checkout_node)
    graph.add_node("deeplink_checkout", deeplink_checkout_node)
    graph.add_node("headless_checkout", headless_checkout_node)
    graph.add_node("handle_error", error_handler_node)

    graph.add_conditional_edges(
        "api_checkout",
        route_after_checkout,
        {"success": END, "fallback": "deeplink_checkout", "error": "handle_error"}
    )
    # Additional edge definitions for each transition...
    return graph.compile()
```

```python
# LangChain approach (what we shipped)
def checkout_with_fallback(cart_items: List[CartItem]) -> CheckoutResult:
    result = try_api_checkout(cart_items)
    if result.success:
        return result

    result = try_deeplink_checkout(cart_items)
    if result.success:
        return result

    return try_headless_checkout(cart_items)
```

The LangGraph version required state schema definitions, node implementations, edge routing functions, and graph compilation. The LangChain version was immediately readable and debuggable. When something failed, we knew exactly where to look.
...
> "I think they're really bad. My opinion. But there's a couple, some very obvious gaps there between context management and all of that. They're very generic solutions. So what we have done is built our own framework on top of the OpenAI SDK. And we are like, three to four times faster than LangChain."
> -- Sahil, Clientell

The three to four times performance difference that Sahil measured aligns with what we observed qualitatively. Framework overhead accumulates across every interaction, every state transition, every tool call. For a shopping agent processing search queries, comparisons, and checkout flows, this overhead translates directly to latency and cost.

The Manus fireside conference provided quantitative framing for these observations:
