...
- **Saurav (RunLLM)** — MCP ecosystem immature, frameworks (LangGraph, Agentic AI) reduce control, teams prefer building in-house to "build intuition"
- **Tushar (CodeForge)** — MCP confuses agents with too many tools, harness obsolescence as models improve, coding agents succeed without frameworks
- **Mrinal (Autonomy)** — Positioning in "missing middle" between low-code (lovable) and infrastructure (AWS/LangChain); actor model abstraction for distributed agents
- **CC (Vivgrid)** — MCP scalability failure at 25+ tools (30% accuracy drop); framework bloat; dynamic tool loading via SLM fine-tuning as solution
- **Sahil (Clientell)** — Custom framework 3-4x faster than LangChain; framework abandonment for production; "nobody we've spoken to seems to be using frameworks"
- **Mehak (Sybill)** — MCP dismissed as "thin value add"; custom embedding models; skepticism of Neo4j knowledge graphs; focus on data architecture over frameworks
- **Abhishek (Apna)** — Memory solutions (MEMZero, Leta, Cognivian) still immature; no out-of-box production-scale agentic infrastructure; significant custom engineering required
...
- **Yujian Tan** — Strong critique: "wouldn't build production agent with LangChain or existing frameworks"; prompt bloat problem; coding agents work, others limited
- **Cynthia (WiseAgents)** — 80-90% abandon LangChain after starting with it; framework bloat when moving from prototype to production; custom solutions required
...
- **CrewAI (Joao)** — AgentScope framework for multi-agent orchestration; open source as enterprise acquisition channel; framework handles "missing middle" complexity
...
- **Manus Fireside** — Multi-model orchestration (Gemini/GPT-4/Claude); framework contributes only 30-40% (models 30-40%, system architecture 60-70%); custom solutions
...
- **Shopping Agent** — LangGraph → LangChain switch due to "extensive bloat and complexity"; firsthand validation of framework overhead; MCP mocked (not production-ready)
...
- **Good Agents** — LangChain for orchestration successful in specific use case; LiteLLM for multi-provider abstraction; comprehensive observability stack (OpenTelemetry, Jaeger, Langfuse, Prometheus, Grafana)

## Synthesized Finding

The framework and tooling ecosystem for agentic AI reveals a critical **adoption-abandonment paradox**: frameworks like LangChain achieve billion-dollar valuations through initial developer adoption, yet 80-90% of production teams abandon them due to bloat, performance overhead (3-4x slower), and loss of control. The "missing middle" problem persists—simple tools (n8n, lovable) cannot scale to production, while complex infrastructure (LangChain, LangGraph) imposes excessive abstraction. Teams consistently build custom solutions optimized for their use cases, with frameworks contributing only 30-40% to agent success versus system architecture's 60-70%. Critical gaps remain in observability (no standardized agent action logging), evaluation tools (limited adoption despite 7+ YC companies), and MCP scalability (accuracy drops to 30% beyond 25 tools).

## Supporting Evidence

### Framework Abandonment Pattern

> "Every company we've talked to, they stop. They started with lang chain as a framework to build AI agents. But once they start going into customers and into production, they realize, like, oh, no. It's full of bloat. Like, it has a lot of unnecessary things. They end up ditching dissolution, and they build their own. This has been, like, 80, 90% of the clients we've we've talked to." — Cynthia (WiseAgents)

> "I wouldn't build an agent and put it into production with LangChain or any of the existing frameworks." — Yujian Tan

> "During development, we initially implemented the agent using LangGraph but switched to LangChain due to extensive bloat and complexity." — Shopping Agent Prototype

> "I think they're really bad. My opinion. But there's a couple, some very obvious gaps there between context management and all of that. They're very generic solutions. So what we have done is built our own framework on top of the Open SDK. And we are like, three to four times faster than LangChain." — Sahil (Clientell)

### Framework vs. Custom Performance

> "We found out actually model only maybe contributes 30 or 40% of the whole thing. And the framework, the whole system you build upon the model is much more important, you know, than the model itself." — Manus Fireside

> "The instinct is to at least start it in house just to build the intuition... I feel like A, they [frameworks] weren't giving us the amount of control we wanted and B, it was hard for the team to just build intuition of, like, how should we be thinking about agentic systems?" — Saurav (RunLLM)
...
> "When you have more than 20 MCP like 30 40, the agent totally cannot work. We were tested and some posts on Internet test that if your MCP amount exceeds 25, your LM accuracy will drop to 30%. So that's totally cannot use in production for enterprise usage." — CC (Vivgrid)

> "There was a lot of talk around MCP kind of solving for everybody. We quickly learned that it didn't work out of the box for us. A, there were just deployment challenges. Like the MCP ecosystem was very immature at the time. And then also the MCPs that either companies are putting up themselves or frankly at the time there was a lot of like open source third party MCP servers... they had way too much like way too many operations specified. There is no specificity." — Saurav (RunLLM)

> "MCP's sort of abstracted out that whole idea of tool calling... And it works, but it doesn't work that well. In fact, it sort of confuses the agent a lot of times than really helping." — Tushar (CodeForge)

### Observability Gaps

> "I think a lot of companies that are doing agents, there's not like a standardized way of logging what are the exact actions and being able to do like really like detailed debugging and maybe even like, quote unquote, time traveling of like, why did the agent take this action at this moment in time?" — Saurav (RunLLM)
...
> "I think their [Letta's] approach takes away too much control. I think our team would personally find that like we want more control over our management system... I would want some sort of solution that's like not that hands off because I think that a system I don't have as much configurability into." — Saurav (RunLLM)

> "The thing that we worry about is just the explosion of noise. So if there's slot that's put into the market maps, then it will compound. And so we're careful about that." — VCatTheory

## Sub-Themes Identified

### 1. LangChain Adoption and Abandonment Patterns

- **Initial adoption**: 80-90% start with LangChain/LangGraph for prototyping
...
- **Performance penalties**: LangChain 3-4x slower than custom implementations
...
- **Hybrid approach**: Use framework concepts, implement custom (Roblox, CrewAI examples)
...
- **Open source for acquisition**: CrewAI uses OSS for prototyping phase, monetizes production
...
- **Pro-framework**: CrewAI, Roblox demonstrate successful LangGraph usage for orchestration
- **Anti-framework**: 80-90% abandon LangChain for production; custom builds 3-4x faster
...
- **Optimists**: Anthropic positioning MCP as standardization breakthrough; Tian (Okta) working on identity extensions
...
- **Reality**: 30-40% model, 60-70% system architecture (Manus empirical data)
...
- **Consumer success**: ChatPRD, Manus used build-in-public for viral growth
...
### Shopping Agent: LangGraph → LangChain Framework Switch

**Validation**: The research team's firsthand experience **strongly confirms** interview findings about framework bloat. Under time pressure (SF Tech Week), LangGraph's abstractions became intolerable, forcing mid-development switch to LangChain. This is direct, experiential evidence rather than reported observation.
...
**Validation**: **LangChain works well for orchestration** in multi-agent systems when applied selectively. Contrasts with Shopping Agent's negative experience, revealing context-dependency of framework value.
...
LangChain's $1.3B valuation based on initial adoption metrics masks 80-90% production abandonment rate. This suggests venture capital evaluation of developer tools conflates early experimentation with sustained usage. Recommendation: Evaluate agent infrastructure companies on retention cohorts, not DAU/MAU.

### 2. The "Missing Middle" is a $10B+ Opportunity

Gap between simple tools (n8n, lovable) and complex infrastructure (LangChain, AWS) creates opportunity for production-grade agent platforms. Successful solution must balance abstraction (developer velocity) with control (production requirements). Autonomy, CrewAI positioning here; market still nascent.
...
MCP's over-generality (25-tool limit, 30% accuracy drop) can be solved via dynamic tool loading: SLM fine-tuning with ~50 examples to select relevant tools pre-invocation. This addresses fat client architecture while preserving standardization benefits. Vivgrid demonstrates pattern; broader adoption needed.
...
Nuanced position: frameworks valuable for orchestration (Good Agents/LangChain success) but harmful for simple tool calling (Shopping Agent abandonment). Context-dependent adoption required. Recommendation: Use frameworks for complex coordination; build custom for performance-critical or simple paths.

### 10. Open Source as Enterprise Acquisition Strategy

CrewAI and Alibaba Qwen demonstrate pattern: open source for negative-ROI prototyping phase, monetize positive-ROI production deployment. Addresses enterprise customization needs while building ecosystem. Recommendation: Open source is viable path if commercial model targets production scale, not prototyping.
