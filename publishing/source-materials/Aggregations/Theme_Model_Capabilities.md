...
- **Tushar (CodeForge)** — Coding agents are exceptional use case; models contribute less than expected; generation vs analysis asymmetry is fundamental
- **Tian (Okta)** — Coding agents show 3x productivity gains; generic tasks 3-5x more expensive than human labor; domain specificity critical
- **Saurav (RunLLM)** — Tool calling specificity challenges; context window management with logging systems; sub-agent patterns for compression
- **CC (Vivgrid)** — MCP scalability breaks at 25+ tools (30% accuracy drop); dynamic tool loading via SLM fine-tuning
- **Sahil (Clientell)** — LLMs terrible at tabular data; visual structure conversion needed; legacy platform specialization gap
- **Mehak (Sybill)** — Generation is foundation model territory; retrieval/context is competitive advantage; models contribute 30-40% to success
- **Abhishek (Apna)** — Fine-tuning for industry-specific accuracy; voice AI latency challenges; coding agents not reliable for complex codebases
- **Stephen (GMI)** — LLM-as-judge limitations; browser control and tool usage as emerging capabilities
- **Yujian Tan** — Coding agents dominant production use case; other agent types struggling
...
- **VCatTheory (VC)** — Model capabilities plateau emerging; constellation of fine-tuned small models (8B params) for 100x improvement
...
- **Manus Fireside** — Model contribution only 30-40% vs initial 70% belief; multi-model orchestration strategy (Gemini/GPT-4/Claude)
...
> — Tian (Okta), describing 3x productivity gains with coding agents (Devin, Windsurf)
...
> — Manus Co-Founder, on team transformation with coding agents
...
> — Tushar (CodeForge), explaining generation vs analysis gap
...
> — David (Qurrent), asserting GPT-4 level models already sufficient
...
> — Tian (Okta), on cost disadvantage for analysis-heavy tasks
...
> — Manus Co-Founder, explaining task-specific model selection
...
> — Autonomy CTO, challenging value of large context windows
...
> — Sahil (Clientell), on data structure innovation for LLM consumption
...
> — Sahil (Clientell), on legacy platform specialization gap
...
> — CC (Vivgrid), on MCP scalability limitations
...
- **Evidence**: 3x productivity gains (Okta), 80% AI-generated code (Manus), 20→150 integrations (Tian)
...
- **Production pattern**: Break into phases with different models per phase (Manus approach)
...
- **Position A (David/Qurrent, Mrinal/Autonomy)**: Current models are "good enough"; bottleneck is deployment/expectation management, not capability
- **Position B (Tushar/CodeForge, Summer/Harvey)**: Models need to be "significantly better" for non-coding agents; generation vs analysis gap is fundamental
...
- **Initial belief**: Models contribute 70% to agent success (Manus, multiple founders)
...
- **Tushar's NASA analogy**: Waiting for better "propulsion" (models) may be faster than building with current capabilities
...
- LangGraph→LangChain switch due to "extensive bloat and complexity"
