# Deliverables Backup Manifest

## Project: AI Agents Research Publishing

**Backup Date:** December 3, 2025
**Total Size:** 748 KB

## Directory Structure

```
/publishing/deliverables/
├── blog-posts/ (26 files, 404 KB)
├── linkedin-posts/ (26 files, 108 KB)
├── images/ (6 files, 72 KB)
└── [18 root files]
```

## File Inventory

### Blog Posts (26 files)

| Filename                                  | Size         | Last Modified    |
| ----------------------------------------- | ------------ | ---------------- |
| 2025-12-40-percent-context-rule.mdx       | 13,082 bytes | 2025-12-03 20:57 |
| 2025-12-ai-agents-research-overview.mdx   | 10,137 bytes | 2025-12-03 20:43 |
| 2025-12-autonomy-interview.mdx            | 14,916 bytes | 2025-12-03 21:52 |
| 2025-12-coding-agent-exception.mdx        | 14,388 bytes | 2025-12-03 21:15 |
| 2025-12-component-evaluation.mdx          | 12,417 bytes | 2025-12-03 21:45 |
| 2025-12-crewai-interview.mdx              | 14,961 bytes | 2025-12-03 22:00 |
| 2025-12-demo-production-chasm.mdx         | 18,118 bytes | 2025-12-03 21:07 |
| 2025-12-dual-memory-architecture.mdx      | 18,294 bytes | 2025-12-03 21:41 |
| 2025-12-enterprise-business-case.mdx      | 14,278 bytes | 2025-12-03 21:12 |
| 2025-12-evaluation-gap.mdx                | 15,766 bytes | 2025-12-03 21:46 |
| 2025-12-framework-abandonment.mdx         | 15,947 bytes | 2025-12-03 21:03 |
| 2025-12-good-agents.mdx                   | 16,003 bytes | 2025-12-03 22:23 |
| 2025-12-handoff-rate-metric.mdx           | 11,630 bytes | 2025-12-03 21:42 |
| 2025-12-manus-fireside.mdx                | 12,304 bytes | 2025-12-03 22:29 |
| 2025-12-mcp-tool-cliff.mdx                | 15,644 bytes | 2025-12-03 21:42 |
| 2025-12-model-myth.mdx                    | 14,160 bytes | 2025-12-03 21:24 |
| 2025-12-okta-interview.mdx                | 14,417 bytes | 2025-12-03 21:58 |
| 2025-12-production-summit.mdx             | 14,527 bytes | 2025-12-03 22:39 |
| 2025-12-qurrent-interview.mdx             | 15,281 bytes | 2025-12-03 21:53 |
| 2025-12-repo-patcher.mdx                  | 17,003 bytes | 2025-12-03 22:18 |
| 2025-12-research-methodology.mdx          | 13,827 bytes | 2025-12-03 22:42 |
| 2025-12-shopping-agent.mdx                | 15,733 bytes | 2025-12-03 22:09 |
| 2025-12-sybill-interview.mdx              | 11,694 bytes | 2025-12-03 22:03 |
| 2025-12-system-integration-92-percent.mdx | 11,890 bytes | 2025-12-03 20:53 |
| 2025-12-why-95-fail.mdx                   | 13,759 bytes | 2025-12-03 22:32 |
| README.md                                 | 5,488 bytes  | 2025-12-03 22:51 |

### LinkedIn Posts (26 files)

| Filename                     | Size        | Last Modified    |
| ---------------------------- | ----------- | ---------------- |
| 01-anchor-post.md            | 1,235 bytes | 2025-12-03 20:42 |
| 02-system-integration.md     | 1,849 bytes | 2025-12-03 20:50 |
| 03-context-management.md     | 1,181 bytes | 2025-12-03 20:55 |
| 04-framework-abandonment.md  | 1,211 bytes | 2025-12-03 21:02 |
| 05-demo-production-chasm.md  | 1,349 bytes | 2025-12-03 21:07 |
| 06-enterprise-blockers.md    | 1,360 bytes | 2025-12-03 21:12 |
| 07-coding-agent-exception.md | 1,430 bytes | 2025-12-03 21:20 |
| 08-model-myth.md             | 1,246 bytes | 2025-12-03 21:20 |
| 09-handoff-rate.md           | 2,203 bytes | 2025-12-03 21:43 |
| 10-dual-memory.md            | 2,280 bytes | 2025-12-03 21:41 |
| 11-mcp-tool-cliff.md         | 2,200 bytes | 2025-12-03 21:44 |
| 12-component-evaluation.md   | 2,284 bytes | 2025-12-03 21:44 |
| 13-evaluation-gap.md         | 2,431 bytes | 2025-12-03 21:44 |
| 14-qurrent-interview.md      | 2,521 bytes | 2025-12-03 21:52 |
| 15-autonomy-interview.md     | 1,669 bytes | 2025-12-03 21:52 |
| 16-okta-interview.md         | 2,854 bytes | 2025-12-03 21:55 |
| 17-crewai-interview.md       | 3,029 bytes | 2025-12-03 22:00 |
| 18-sybill-interview.md       | 3,053 bytes | 2025-12-03 22:05 |
| 19-shopping-agent.md         | 2,779 bytes | 2025-12-03 22:05 |
| 20-repo-patcher.md           | 2,847 bytes | 2025-12-03 22:12 |
| 21-good-agents.md            | 3,243 bytes | 2025-12-03 22:21 |
| 22-manus-fireside.md         | 2,447 bytes | 2025-12-03 22:27 |
| 23-why-95-fail.md            | 2,645 bytes | 2025-12-03 22:35 |
| 24-production-summit.md      | 3,017 bytes | 2025-12-03 22:36 |
| 25-research-methodology.md   | 2,817 bytes | 2025-12-03 22:41 |
| README.md                    | 4,582 bytes | 2025-12-03 22:50 |

### Image Specs (6 files)

| Filename                                | Size         | Last Modified    |
| --------------------------------------- | ------------ | ---------------- |
| anchor-cover-spec.md                    | 4,708 bytes  | 2025-12-03 22:42 |
| infographic-70-30-spec.md               | 8,435 bytes  | 2025-12-03 22:45 |
| infographic-enterprise-blockers-spec.md | 10,687 bytes | 2025-12-03 22:45 |
| infographic-framework-spec.md           | 9,873 bytes  | 2025-12-03 22:45 |
| infographic-metrics-spec.md             | 8,476 bytes  | 2025-12-03 22:45 |
| theme-covers-spec.md                    | 16,328 bytes | 2025-12-03 22:45 |

### Documentation & Reports (18 files)

| Filename                     | Size         | Last Modified    |
| ---------------------------- | ------------ | ---------------- |
| claude-progress.txt          | 4,666 bytes  | 2025-12-03 22:53 |
| cross-linking-report.md      | 8,159 bytes  | 2025-12-03 22:56 |
| final-checklist.md           | 3,192 bytes  | 2025-12-03 22:53 |
| homepage-featured.md         | 5,585 bytes  | 2025-12-03 22:55 |
| publication_tracker.json     | 7,609 bytes  | 2025-12-03 22:53 |
| sitemap-rss-spec.md          | 17,763 bytes | 2025-12-03 22:56 |
| social-metadata-spec.md      | 10,089 bytes | 2025-12-03 22:56 |
| source-archive-manifest.md   | 3,290 bytes  | 2025-12-03 22:53 |
| validation-blog-posts.md     | 4,645 bytes  | 2025-12-03 22:47 |
| validation-linkedin-posts.md | 5,282 bytes  | 2025-12-03 22:47 |
| validation-links.md          | 10,733 bytes | 2025-12-03 22:51 |
| validation-privacy.md        | 5,151 bytes  | 2025-12-03 22:48 |
| validation-proofreading.md   | 4,626 bytes  | 2025-12-03 22:50 |
| validation-quotes.md         | 13,934 bytes | 2025-12-03 22:48 |
| validation-statistics.md     | 5,584 bytes  | 2025-12-03 22:47 |
| validation-wordcount.md      | 7,286 bytes  | 2025-12-03 22:50 |
| website-copy-instructions.md | 11,770 bytes | 2025-12-03 22:56 |
| website-env-check.md         | 3,842 bytes  | 2025-12-03 22:53 |

## Total File Count

| Category                      | Files        |
| ----------------------------- | ------------ |
| Blog posts (.mdx + README)    | 26           |
| LinkedIn posts (.md + README) | 26           |
| Image specs                   | 6            |
| Documentation & Reports       | 18           |
| **Total**                     | **76 files** |

## Backup Commands

### Create Compressed Archive

```bash
cd /Users/fernandotn/Downloads/390
tar -czvf ai-agents-research-backup-2025-12-03.tar.gz publishing/deliverables/
```

### Create Zip Archive (Alternative)

```bash
cd /Users/fernandotn/Downloads/390
zip -r ai-agents-research-backup-2025-12-03.zip publishing/deliverables/
```

### Copy to External Location

```bash
cp -R /Users/fernandotn/Downloads/390/publishing/deliverables/ /path/to/backup/location/
```

## Verification

### Verify Archive Integrity

```bash
# For tar.gz
tar -tzvf ai-agents-research-backup-2025-12-03.tar.gz | wc -l
# Should return: 76+ (files plus directories)

# List contents
tar -tzvf ai-agents-research-backup-2025-12-03.tar.gz
```

### Verify File Count After Extraction

```bash
find /path/to/extracted/deliverables -type f | wc -l
# Should return: 76
```

### Checksum Generation (Optional)

```bash
# Generate checksums for all files
cd /Users/fernandotn/Downloads/390/publishing/deliverables
find . -type f -exec md5 {} \; > ../backup-checksums.md5

# Verify after restore
md5 -c ../backup-checksums.md5
```

## Content Summary

This backup contains the complete deliverables for the AI Agents Research Publishing Project:

- **25 blog posts** covering AI agent research findings, interviews, and case studies
- **25 LinkedIn posts** for social media promotion of each blog post
- **6 image specification files** for infographics and cover images
- **8 validation reports** ensuring content quality
- **10 supporting documents** including trackers, specs, and instructions

All files were created on December 3, 2025, and represent the complete publication-ready content package.

---

_Manifest generated: December 3, 2025_
